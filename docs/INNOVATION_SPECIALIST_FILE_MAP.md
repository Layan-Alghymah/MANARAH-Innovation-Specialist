...
| `app/innovation-specialist/evaluations/page.tsx` | Evaluation queue route | Copy directly | Same route | Low | Thin wrapper. |
| `app/innovation-specialist/evaluations/evaluations-client.tsx` | Interactive evaluation queue | Copy directly | Same route or module components | Medium | Preserve query-tab destinations. |
...
| `mock-data/index.ts` | Dashboard, ideas, evaluations, projects, and detail mocks | Temporary mock-only file | Temporary test/fixture location | Low | Remove after API integration; never use as production source. |
