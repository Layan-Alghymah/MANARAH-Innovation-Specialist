...
export function ProjectsClient({ initialFilter = "all" }: { initialFilter?: string }) {
  ...
  const managers = useMemo(() => Array.from(new Set(assignedProjects.map(project => project.manager))), []);

  const filtered = useMemo(() => assignedProjects.filter(project => {
  ...
  const attention = assignedProjects.filter(project => project.attentionReason).slice(0, 4);
  ...
  return <div className="mx-auto max-w-[1600px]">
    ...
    <p className="mt-5 text-sm text-slate-500">عرض <strong className="text-slate-900">{filtered.length}</strong> من أصل {assignedProjects.length} مشروعًا مسندًا للمتابعة</p>
