...
import { ChevronLeft, ChevronRight, FilterX, ListChecks, SearchX } from "lucide-react";
...
type FilterKey = "all" | "waiting-review" | "needs-information" | "under-evaluation" | "recommendation-ready" | "recommendation-submitted" | "returned";
...
const actions: Record<IdeaStatus, string> = {
  ...
  "تم رفع التوصية": "عرض التوصية",
  "معادة للمراجعة": "معالجة الملاحظات"
};

const actionTabs: Partial<Record<IdeaStatus, "evaluation" | "communication" | "recommendation">> = {
  "تحتاج استكمال": "communication",
  "قيد التقييم": "evaluation",
  "جاهزة للتوصية": "recommendation",
  "تم رفع التوصية": "recommendation",
  "معادة للمراجعة": "recommendation"
};

const summaryItems: Array<{ label: string; filter: FilterKey; icon: typeof ListChecks; count: (rows: IdeaListItem[]) => number }> = [
  ...
  { label: "جاهزة لرفع التوصية", filter: "recommendation-ready", icon: ListChecks, count: rows => rows.filter(row => row.status === "جاهزة للتوصية").length },
  { label: "تم رفع التوصية", filter: "recommendation-submitted", icon: ListChecks, count: rows => rows.filter(row => row.status === "تم رفع التوصية").length },
  { label: "معادة للمراجعة", filter: "returned", icon: ListChecks, count: rows => rows.filter(row => row.status === "معادة للمراجعة").length }
];
...
export function IdeasClient({ initialFilter }: { initialFilter: string }) {
  ...
  const categories = useMemo(() => Array.from(new Set(assignedIdeas.map(idea => idea.category))), []);
  const filteredIdeas = useMemo(() => assignedIdeas.filter(idea => {
    ...
    const matchesMainFilter = activeFilter === "all" || idea.status === expectedStatus;
  ...
  const openIdea = (id: string, tab?: "evaluation" | "communication" | "recommendation") =>
    router.push(`/innovation-specialist/ideas/${id}${tab ? `?tab=${tab}` : ""}`);
  ...
  return <div className="mx-auto max-w-[1600px]">
    ...
    <div className="mt-5 flex items-center justify-between gap-3"><p className="text-sm text-slate-500">عرض <span className="font-semibold text-slate-900">{filteredIdeas.length}</span> من أصل {assignedIdeas.length} فكرة مسندة</p></div>
    ...
    <div className="mt-3">
      {visibleIdeas.length > 0 ? <DataTable headers={["عنوان الفكرة", "المبتكر", "نوع المبتكر", "التصنيف", "الحالة", "الأولوية", "تاريخ الإسناد", "آخر تحديث", "الإجراء"]}>
        {visibleIdeas.map(idea => <tr key={idea.id} tabIndex={0} onClick={() => openIdea(idea.id)} onKeyDown={event => { if (event.key === "Enter") openIdea(idea.id); }} className="cursor-pointer border-t border-slate-100 transition-colors hover:bg-slate-50 focus:bg-brand-50/50 focus:outline-none">
          ...
          <td className="whitespace-nowrap px-4 py-4"><Button type="button" className="min-h-9 px-3 text-xs" onClick={event => { event.stopPropagation(); openIdea(idea.id, actionTabs[idea.status]); }}>{actions[idea.status]}</Button></td>
        </tr>)}
