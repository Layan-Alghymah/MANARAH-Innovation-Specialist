...
export default function DashboardPage() {
  return <div className="mx-auto max-w-[1600px]">
    ...
    <section className="mt-8">
      ...
      <DataTable headers={["الفكرة", "المبتكر", "التصنيف", "الحالة الحالية", "الموعد", "الأولوية", "الإجراء"]}>
        {dashboardTasks.map(task => <tr key={task.id} className="border-t border-slate-100 transition-colors hover:bg-slate-50/70">
          <td className="min-w-64 px-4 py-4"><Link href={task.id.startsWith("PRJ") ? `/innovation-specialist/projects/${task.id}` : `/innovation-specialist/ideas/${task.id}`} className="font-semibold text-slate-900 hover:text-brand-700">{task.title}</Link><span className="mt-1 block text-xs text-slate-400">{task.id}</span></td>
          <td className="whitespace-nowrap px-4 py-4 text-slate-600">{task.innovator}</td>
          ...
          <td className="whitespace-nowrap px-4 py-4"><Link className={actionClass} href={task.id.startsWith("PRJ") ? `/innovation-specialist/projects/${task.id}` : `/innovation-specialist/ideas/${task.id}`}>{task.action}</Link></td>
        </tr>)}
      </DataTable>
    </section>
    ...
    <div className="mt-8 grid items-start gap-8 xl:grid-cols-[minmax(0,1.55fr)_minmax(340px,0.75fr)]">
      <section>
        <SectionHeading title="الأفكار المسندة حديثًا" description="أحدث الأفكار التي أُضيفت إلى قائمة عملك." href="/innovation-specialist/ideas?filter=assigned" linkLabel="عرض جميع الأفكار" />
        ...
        <Card className="p-0">
          <div className="divide-y divide-slate-100">
            {dashboardNotifications.map(notification => {
              ...
              return <Link href="/innovation-specialist/notifications" key={notification.id} className="flex gap-3 p-4 transition-colors first:rounded-t-xl last:rounded-b-xl hover:bg-slate-50">
